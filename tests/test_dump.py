...
def test_dump_with_doc():
    result = r.invoke(cli, ["dump", "tests/demo.doc"])
    assert result.exit_code == 0
    assert '"status": 200' in result.output


def test_dump_strip():
    result = r.invoke(cli, ["dump", "-s", 50, "tests/demo.doc"])
    assert result.exit_code == 0
...
def test_dump_meta_only():
    result = r.invoke(cli, ["dump", "-m", "tests/demo.doc"])
    assert result.exit_code == 0
    data = json.loads(result.output)
...
def test_dump_usage_error():
    result = r.invoke(cli, ["dump", "-m", "-c", "tests/demo.doc"])
